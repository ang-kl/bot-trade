  ...
  return (
    <Card>
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <div className="text-(length:--fs-body) font-semibold">Market sessions</div>
        ...
        {SESSIONS.map(s => {
          ...
          return (
            <div key={s.id} className="flex items-center gap-1.5 rounded-[8px] glass-inset px-2 py-1">
              ...
              <span className="text-(length:--fs-body) font-semibold">{s.label}</span>
              <span className="text-(length:--fs-body) text-[var(--color-text-sub)] tabular-nums">{fmtInTz(now, s.tz)}</span>
            </div>
          )
        })}
      </div>
