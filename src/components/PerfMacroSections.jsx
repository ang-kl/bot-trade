  ...
  return (
    <div style={{ flex: 1, border: `1px solid ${q.bd}`, background: q.bg, borderRadius: 14, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 3 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
        <span style={{ fontSize: 9.5, fontWeight: 800 }}>{q.title}</span>
        <span style={{ marginLeft: 'auto', fontSize: 9.5, fontWeight: 800, fontVariantNumeric: 'tabular-nums', color: MU }}>{q.tot}</span>
      </div>
      <span style={{ fontSize: 9.5, lineHeight: 1.4, color: SB }}>{q.txt}</span>
      {q.rows.map((r, ri) => (
        <div key={ri} style={{ display: 'flex', alignItems: 'baseline', gap: 5, borderTop: `1px solid ${EDG}`, paddingTop: 3, fontVariantNumeric: 'tabular-nums' }}>
          ...
          <span style={{ fontSize: 9.5, color: SB }}>{r.sd}</span>
          <span style={{ fontSize: 9.5, color: MU }}>{r.acct}</span>
          <span style={{ marginLeft: 'auto', fontSize: 9.5, fontWeight: 800, color: MU }}>{r.pnl}</span>
        </div>
      ))}
      {q.rows.length === 0 && <span style={{ fontSize: 9.5, color: MU, borderTop: `1px solid ${EDG}`, paddingTop: 3 }}>no open trades in this quadrant</span>}
    </div>
  )
}
  ...
  return (
    <div style={{ ...panel, gap: 2 }}>
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
        <span style={{ fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em', color: MU }}>Account</span>
        {rOpts.map(o => {
          const on = rAcct === o.id
          return (
        ...
        <span style={{ fontSize: 9.5, color: MU }}>filters the four quadrant cards — where this account's open trades sit right now{unmapped ? ` · ${unmapped} open position${unmapped === 1 ? '' : 's'} outside the quadrant map` : ''}</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '330px 1fr 330px', gap: 10, alignItems: 'stretch' }}>
        ...
        <svg viewBox="0 0 720 620" style={{ width: '92%', justifySelf: 'center', alignSelf: 'center', overflow: 'visible', aspectRatio: '3 / 2', minHeight: 520 }}>
          <g>
            <text x="30" y="52" fontSize="9.5" fontWeight="700" fill={SB}>Lo Vol</text>
            ...
            <text x="132" y="52" fontSize="9.5" fontWeight="700" fill={DN}>Hi Vol</text>
          </g>
          ...
          <text x="30" y="30" fontSize="9.5" fontWeight="800" fill={MU}>Q2 · Stagflation</text>
          <text x="690" y="30" textAnchor="end" fontSize="9.5" fontWeight="800" fill={MU}>Q1 · Overheating</text>
          <text x="30" y="600" fontSize="9.5" fontWeight="800" fill={MU}>Q3 · Deflation</text>
          <text x="690" y="600" textAnchor="end" fontSize="9.5" fontWeight="800" fill={MU}>Q4 · Goldilocks</text>
          {dots.map(p => {
            ...
            return (
              <g key={p.name} style={{ cursor: 'pointer', opacity: dim ? 0.35 : 1 }}
                ...
                <text x={p.tx} y={p.cy} textAnchor={p.anc} dominantBaseline="middle" fontSize="10" fontWeight={sel ? 800 : 700} fill={TX} stroke="var(--color-bg)" strokeWidth="2.5" paintOrder="stroke" fontVariant="tabular-nums">{p.key} <tspan fontWeight="800" fill={p.col}>{p.pnl}</tspan></text>
              </g>
            )
          })}
        </svg>
        ...
        {dots.map(d => {
          ...
          return (
            <button key={d.key} type="button"
              ...
              <span style={{ fontSize: 10, color: TX }}>{d.name}</span>
              <span style={{ fontSize: 10, fontWeight: 800, fontVariantNumeric: 'tabular-nums', textAlign: 'right', color: d.col }}>{d.pnl}</span>
              <span style={{ fontSize: 9.5, color: MU, textAlign: 'right' }}>{d.volBand}</span>
            </button>
          )
        })}
        <span style={{ gridColumn: '1 / -1', fontSize: 9.5, color: MU }}>P · Long commodities/TIPS — playbook reference only, no traded group is plotted for it</span>
      </div>
      <span style={{ fontSize: 9.5, color: MU }}>positions are the classic growth × inflation playbook for each group the bot trades — center = cash-like calm, outer ring = highest volatility · blue label = group is net positive over 30D, red = net negative</span>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, borderTop: `1px solid ${EDG}`, paddingTop: 5 }}>
        {OIA.map(([title, txt, topBorder]) => (
          <div key={title} style={{ display: 'flex', flexDirection: 'column', gap: 2, ...(topBorder ? { borderTop: `1px solid ${EDG}`, paddingTop: 4 } : {}) }}>
            ...
            <span style={{ fontSize: 9.5, lineHeight: 1.45, color: SB }}>{txt}</span>
          </div>
        ))}
      </div>
    </div>
...
export function BalanceInOut({ inModal = false }) {
  return (
    <div style={{ ...panel, gap: 3 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '120px 150px 110px 130px 1fr 110px 70px', gap: 8, fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: MU, borderBottom: `1px solid ${EDG}`, paddingBottom: 2 }}>
...
export function DataFeed({ balance, freeMargin, equity, openCount, dailyLossPct, equityStopArmed, slSet, tpSet, clock, inModal = false }) {
  ...
  const chip = { fontSize: 9.5, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: ACS, border: `1px solid ${GBD}` }
  ...
  return (
    <div style={{ ...panel, gap: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
        <div style={box}>
          <span style={{ fontSize: 9.5, fontWeight: 800 }}>OHLCV data</span>
          <span style={{ fontSize: 9.5, color: SB, lineHeight: 1.4 }}>Open · High · Low · Close · Volume across multiple timeframes</span>
          <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
            {['1m', '15m', '1h', '4h', '1D'].map(tf => <span key={tf} style={chip}>{tf}</span>)}
          </div>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>watchlist symbols stream per scan cycle · {clock}</span>
        </div>
        <div style={box}>
          <span style={{ fontSize: 9.5, fontWeight: 800 }}>Account &amp; portfolio state</span>
          <span style={{ fontSize: 9.5, color: SB, lineHeight: 1.4 }}>Live cash, available margin, open positions, unrealized P&amp;L</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>cash {money(balance)} · margin avail {money(freeMargin)}</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>{openCount} open · unrealized <span style={{ fontWeight: 800, color: equity != null && balance != null ? (equity - balance >= 0 ? UP : DN) : MU }}>{equity != null && balance != null ? signed(equity - balance) : '—'}</span></span>
        </div>
        <div style={box}>
          <span style={{ fontSize: 9.5, fontWeight: 800 }}>Execution parameters</span>
          <span style={{ fontSize: 9.5, color: SB, lineHeight: 1.4 }}>Bid-ask spreads, fees, latency, slippage thresholds</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>spread &amp; fees recorded per trade (forensics columns) — no live feed here</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>latency <span style={{ fontWeight: 800, color: ACC }}>—</span> · captured at entry per trade</span>
        </div>
        <div style={box}>
          <span style={{ fontSize: 9.5, fontWeight: 800 }}>Risk controls</span>
          <span style={{ fontSize: 9.5, color: SB, lineHeight: 1.4 }}>Stop-loss limits, take-profit triggers, max drawdown caps</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>SL set {slSet}/{openCount} open · TP set {tpSet}/{openCount}</span>
          <span style={{ fontSize: 9.5, color: MU, fontVariantNumeric: 'tabular-nums' }}>max drawdown {dailyLossPct != null ? `${(dailyLossPct * 100).toFixed(0)}%/day` : '—'} · equity stop <span style={{ fontWeight: 800, color: ACC }}>{equityStopArmed ? 'armed' : 'off'}</span></span>
        </div>
      </div>
    </div>
  )
