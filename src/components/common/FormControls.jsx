...
export function SliderInput({ label, value, onChange, min, max, step, display = v => String(v), parse = v => v, toInput = v => v, unit = '' }) {
  ...
  return (
    <label className="block text-(length:--fs-body)">
      <span className="flex items-baseline justify-between text-[var(--color-text-sub)]">
        ...
        <span className="inline-flex items-center gap-1">
          <input
            ...
            className="glass-inset w-20 rounded-[var(--radius-control)] px-2 py-1 text-right text-(length:--fs-body) font-semibold text-[var(--color-text)] focus:outline-2 focus:outline-[var(--color-accent)] bg-transparent"
            aria-label={`${label} value`}
          />
          {unit && <span className="text-(length:--fs-body)">{unit}</span>}
        </span>
      </span>
      ...
      />
      <span className="flex justify-between text-(length:--fs-body) text-[var(--color-text-sub)]">
...
export function PresetSelect({ label, value, onChange, options, display = v => String(v), parse = v => v, unit = '' }) {
  ...
  return (
    <div className="block text-(length:--fs-body) relative" ref={boxRef}>
      <span className="text-[var(--color-text-sub)]">{label}</span>
      <button
        type="button" aria-haspopup="listbox" aria-expanded={open}
        onClick={() => setOpen(o => !o)}
        className="glass-inset mt-1 w-full rounded-[var(--radius-control)] px-3 py-2 text-left text-(length:--fs-body) min-h-[40px] cursor-pointer flex items-center justify-between gap-2 focus:outline-2 focus:outline-[var(--color-accent)]"
      >
        <span className="font-semibold text-[var(--color-text)]">{current ? current[1] : `${display(num)} (custom)`}</span>
        <span aria-hidden="true" className={`text-(length:--fs-body) text-[var(--color-text-sub)] transition-transform ${open ? 'rotate-180' : ''}`}>▼</span>
      </button>
      {open && (
        <div role="listbox" className="glass-panel pos-absolute is-menu absolute z-30 mt-1 w-full rounded-[12px] p-1.5 shadow-xl">
          {options.map(([v, text]) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left rounded-[var(--radius-control)] px-3 py-2 text-(length:--fs-body) cursor-pointer flex items-center justify-between ${
          ...
          <div className="border-t border-[var(--color-border)] mt-1 pt-1.5 px-1 pb-0.5 flex items-center gap-1.5">
            <input
              ...
              className="glass-inset flex-1 min-w-0 rounded-[var(--radius-control)] px-2 py-1.5 text-(length:--fs-body) bg-transparent focus:outline-2 focus:outline-[var(--color-accent)]"
              aria-label={`Custom ${label}`}
            />
            {unit && <span className="text-(length:--fs-body) text-[var(--color-text-sub)]">{unit}</span>}
            <button type="button" onClick={commitCustom} className="rounded-[var(--radius-control)] bg-[var(--color-accent)] text-white text-(length:--fs-body) font-semibold px-2.5 py-1.5 cursor-pointer">Set</button>
          </div>
        </div>
      )}
    </div>
