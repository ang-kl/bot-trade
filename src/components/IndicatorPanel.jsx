    ...
    <div className="flex flex-wrap items-center gap-1.5 mb-1.5" role="group" aria-label="Chart indicators">
      {INDICATOR_IDS.map(id => {
        ...
        return (
          <button
            ...
            className={`rounded-[8px] px-2 h-[32px] text-(length:--fs-body) font-semibold cursor-pointer border transition-colors ${
      ...
      {prefs.indicators.includes('vp') && (
        <select
          ...
          className="glass-inset rounded-[1px] px-2 min-h-[28px] text-(length:--fs-body) text-[var(--color-text-sub)] bg-transparent cursor-pointer"
        >
      ...
      {prefs.indicators.includes('avwap') && (
        <button
          ...
          onClick={() => onArmAvwap?.()}
          className={`rounded-[1px] px-2 min-h-[28px] text-(length:--fs-body) font-semibold cursor-pointer ${
