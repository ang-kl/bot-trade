  ...
  return (
    <div ref={wrapRef} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        <span style={{ fontSize: 'var(--fs-d9)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em', color: MU }}>Show</span>
        {filters.map(f => (
          <button key={f.id} type="button" onClick={() => setFlt(f.id)} aria-pressed={flt === f.id}
            style={{ cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-d9)', fontWeight: 700, color: f.col, background: f.bg, border: `1px solid ${f.bd}`, borderRadius: 999, padding: '3px 11px' }}>
            {f.label} <span style={{ opacity: 0.65 }}>{f.n}</span>
          </button>
        ))}
        <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-d9)', color: MU }}>closed trades · ✓ passed · ✕ violated · pipeline: pending → live → managed → close</span>
      </div>
      ...
      <div style={{ ...glass, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 1, overflowX: 'auto' }}>
        ...
        {rows.length === 0 && <span style={{ fontSize: 'var(--fs-d9)', color: SB, padding: '6px 0' }}>No closed trades in this bucket yet — rows appear from the first completed round-trip.</span>}
            ...
            <span className="wf" style={{ display: 'flex', alignItems: 'center', padding: '9px 2px 9px', position: 'relative' }}>
              {t.path.map(s => (
                <span key={s.k} style={{ display: 'flex', alignItems: 'center', flex: s.grow, minWidth: 0, position: 'relative' }}>
                  <span style={{ display: s.segShow, position: 'absolute', left: 0, right: 11, top: -9, textAlign: 'center', fontSize: 'var(--fs-d9)', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: s.tCol }}>{s.t}</span>
                  ...
                  <span style={{ position: 'relative', display: 'flex', justifyContent: 'center' }}>
                    ...
                    <span style={{ position: 'absolute', top: 12, fontSize: 'var(--fs-d9)', fontWeight: 700, letterSpacing: '.02em', color: s.col, whiteSpace: 'nowrap' }}>{s.k}</span>
                  </span>
                </span>
              ))}
            </span>
        ...
        {rows.length > PAGE_SIZE && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingTop: 4 }}>
            <button type="button" disabled={page === 0} onClick={() => setPage(p => Math.max(0, p - 1))}
              style={{ cursor: page === 0 ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-d9)', fontWeight: 700, color: page === 0 ? MU : TX, background: 'transparent', border: `1px solid ${EDG}`, borderRadius: 999, padding: '2px 10px' }}>‹ Prev</button>
            ...
            <button type="button" disabled={(page + 1) * PAGE_SIZE >= rows.length} onClick={() => setPage(p => p + 1)}
              style={{ cursor: (page + 1) * PAGE_SIZE >= rows.length ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-d9)', fontWeight: 700, color: (page + 1) * PAGE_SIZE >= rows.length ? MU : TX, background: 'transparent', border: `1px solid ${EDG}`, borderRadius: 999, padding: '2px 10px' }}>Next ›</button>
          </div>
        )}
      </div>
