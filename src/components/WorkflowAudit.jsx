...
export default function WorkflowAudit({ allTrades, postmortems }) {
  ...
  // Owner (2026-07-25): "Trade Audit page have two identical sets of
  // information, remove the first one" — the phone card layout rendered
  // ABOVE the audit table instead of replacing it, so every trade appeared
  // twice. The single table below (overflowX pans on narrow screens) is the
  // one set that remains; all mobile-only derived data went with the layout.
  ...
  return (
    <div ref={wrapRef} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 8 }}>
        <div style={{ ...glass, padding: '8px 11px', display: 'flex', flexDirection: 'column', gap: 3 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <span style={{ fontSize: 'var(--fs-h)', fontWeight: 800, color: ACC }}>Phase 1 · The Lab</span>
            <span style={{ fontSize: 'var(--fs-body)', color: SB }}>research / backtest → strategy</span>
            <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-body)', fontWeight: 800, color: UP }}>{trades.length ? `${nLab}/${trades.length} ✓` : '—'}</span>
          </div>
          <span style={{ fontSize: 'var(--fs-body)', lineHeight: 1.45, color: SB }}><b style={{ color: TX }}>O</b> — historical price action, order blocks, volume profiles mined for a statistical edge; rules frozen into code. <b style={{ color: TX }}>I</b> — backtest proves the past only: it assumes perfect liquidity, zero latency, zero slippage. <b style={{ color: TX }}>A</b> — final parameters: quadrant criteria, entry triggers, risk sizing.</span>
        </div>
        <div style={{ ...glass, padding: '8px 11px', display: 'flex', flexDirection: 'column', gap: 3 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <span style={{ fontSize: 'var(--fs-h)', fontWeight: 800, color: WRN }}>Phase 2 · The Bridge</span>
            <span style={{ fontSize: 'var(--fs-body)', color: SB }}>forward test — the missing step</span>
            <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-body)', fontWeight: 800, color: UP }}>{trades.length ? `${nBridge}/${trades.length} ✓` : '—'}</span>
          </div>
        ...
        <div style={{ ...glass, padding: '8px 11px', display: 'flex', flexDirection: 'column', gap: 3 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <span style={{ fontSize: 'var(--fs-h)', fontWeight: 800, color: VIO }}>Phase 3 · The Market</span>
            <span style={{ fontSize: 'var(--fs-body)', color: SB }}>pending → live → manage → close</span>
            <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-body)', fontWeight: 800, color: UP }}>{trades.length ? `${trades.length - nEbad}/${trades.length} ✓` : '—'}</span>
          </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        <span style={{ fontSize: 'var(--fs-body)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em', color: MU }}>Show</span>
        {filters.map(f => (
          <button key={f.id} type="button" onClick={() => setFlt(f.id)} aria-pressed={flt === f.id}
            style={{ cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-body)', fontWeight: 700, color: f.col, background: f.bg, border: `1px solid ${f.bd}`, borderRadius: 999, padding: '3px 11px' }}>
            {f.label} <span style={{ opacity: 0.65 }}>{f.n}</span>
          </button>
        ))}
        <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-body)', color: MU }}>closed trades · ✓ passed · ✕ violated · pipeline: pending → live → managed → close</span>
      </div>
      ...
      <div style={{ ...glass, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 1, overflowX: 'auto' }}>
        ...
        {rows.length === 0 && <span style={{ fontSize: 'var(--fs-body)', color: SB, padding: '6px 0' }}>No closed trades in this bucket yet — rows appear from the first completed round-trip.</span>}
        {rows.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map(t => (
          <div key={t.id} style={{ display: 'grid', gridTemplateColumns: GRID_COLS, gap: 6, alignItems: 'center', borderBottom: `1px solid ${EDG}`, padding: '4px 0', fontVariantNumeric: 'tabular-nums', background: t.rowBg }}>
            <span style={{ fontSize: 'var(--fs-body)', lineHeight: 1.35, color: SB }}>{t.when}</span>
            <span style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontSize: 'var(--fs-body)', fontWeight: 800 }}>{t.sym}</span>
              <span style={{ fontSize: 'var(--fs-body)', color: t.sideCol }}>{t.sd}</span>
            </span>
            ...
            <span style={{ fontSize: 'var(--fs-body)', color: SB }} title={t.strat}>{strategyLabel(t.strat)}</span>
            <span style={{ fontSize: 'var(--fs-body)', fontWeight: 800, color: t.labCol }}>{t.lab}</span>
            <span style={{ fontSize: 'var(--fs-body)', fontWeight: 800, color: t.brCol }}>{t.br}</span>
            ...
            <span className="wf" style={{ display: 'flex', alignItems: 'center', padding: '9px 2px 9px', position: 'relative' }}>
              {t.path.map(s => (
                <span key={s.k} style={{ display: 'flex', alignItems: 'center', flex: s.grow, minWidth: 0, position: 'relative' }}>
                  <span style={{ display: s.segShow, position: 'absolute', left: 0, right: 11, top: -9, textAlign: 'center', fontSize: 'var(--fs-body)', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: s.tCol }}>{s.t}</span>
                  ...
                  <span style={{ position: 'relative', display: 'flex', justifyContent: 'center' }}>
                    ...
                    <span style={{ position: 'absolute', top: 12, fontSize: 'var(--fs-body)', fontWeight: 700, letterSpacing: '.02em', color: s.col, whiteSpace: 'nowrap' }}>{s.k}</span>
                  </span>
                </span>
              ))}
            </span>
            <span style={{ fontSize: 'var(--fs-body)', fontWeight: 700, color: t.closeCol }}>{t.close}</span>
            <span style={{ fontSize: 'var(--fs-body)', lineHeight: 1.4, color: t.noteCol }}>{t.note}</span>
            <span style={{ fontSize: 'var(--fs-body)', fontWeight: 800, textAlign: 'right', color: t.col }}>{t.pnl}</span>
          </div>
        ))}
        {rows.length > PAGE_SIZE && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingTop: 4 }}>
            <button type="button" disabled={page === 0} onClick={() => setPage(p => Math.max(0, p - 1))}
              style={{ cursor: page === 0 ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-body)', fontWeight: 700, color: page === 0 ? MU : TX, background: 'transparent', border: `1px solid ${EDG}`, borderRadius: 999, padding: '2px 10px' }}>‹ Prev</button>
            ...
            <button type="button" disabled={(page + 1) * PAGE_SIZE >= rows.length} onClick={() => setPage(p => p + 1)}
              style={{ cursor: (page + 1) * PAGE_SIZE >= rows.length ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: 'var(--fs-body)', fontWeight: 700, color: (page + 1) * PAGE_SIZE >= rows.length ? MU : TX, background: 'transparent', border: `1px solid ${EDG}`, borderRadius: 999, padding: '2px 10px' }}>Next ›</button>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {verdicts.map(v => (
          <div key={v.title} style={{ flex: 1, minWidth: 280, background: GL, border: `1px solid ${v.bd}`, borderRadius: 12, padding: '7px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <span style={{ fontSize: 'var(--fs-body)', fontWeight: 800, color: v.tcol }}>{v.title}</span>
              <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-body)', fontWeight: 800, fontVariantNumeric: 'tabular-nums', color: v.ncol }}>{v.stat}</span>
            </div>
            <span style={{ fontSize: 'var(--fs-body)', lineHeight: 1.4, color: SB }}>{v.txt}</span>
          </div>
        ))}
      </div>
      </div>
