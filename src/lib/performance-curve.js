...
export function performanceCurve(report, accountId, dailyDecisions, rangeDays) {
  ...
  const groups = (report?.daily || []).filter(g => (accountId === 'all' || g.accountId === String(accountId)) && Date.parse(g.day) + 86400000 > cutoff)
  ...
  if (Array.isArray(dailyDecisions)) for (const d of dailyDecisions) {
    if (!Number.isFinite(Date.parse(d.day)) || Date.parse(d.day) + 86400000 <= cutoff) continue
