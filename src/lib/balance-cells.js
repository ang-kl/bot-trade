...
export function balanceLines(set, { money = fixed, withCurrency = false, unavailable = 'balance history unavailable' } = {}) {
  ...
  const lines = set.groups.map(g => g.value != null
    ...
    : { key: g.currency, text: `${set.groups.length > 1 || withCurrency ? `${g.currency} ` : ''}${missingBalanceLabel(g)}`,
      title: `${g.currency}: ${missingBalanceLabel(g)}. A missing broker balance is not zero and is never estimated.`, missing: true, currency: g.currency, value: null })
  if (set.unknownCurrencyAccounts > 0) lines.push({ key: 'unknown', missing: true,
    ...
    title: `${set.unknownCurrencyAccounts} account${set.unknownCurrencyAccounts === 1 ? ' has' : 's have'} no stored broker balance, so its currency is unknown and it is in no total.` })
...
export function floatingText(set, { signed = plus } = {}) {
  ...
  const missing = set.groups.filter(g => g.value == null).map(g => `${g.currency} ${missingBalanceLabel(g)}`)
  const title = [...shown.map(g => readTitle(g, 'floating P&L')), ...missing, set.unknownCurrencyAccounts ? `${set.unknownCurrencyAccounts} account(s) with no stored reading` : null]
    .filter(Boolean).join(' · ')
