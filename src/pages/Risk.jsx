...
const SECTIONS = ['risk', 'account', 'guard', 'loss-cap', 'ratchet', 'loss-guardian']
...
export default function Risk() {
  ...
  const [acct, setAcctRaw] = useState({ accountId: null, balance: null, leverage: null })
  ...
  const requestedScope = useRef(riskAcct || 'all')
  requestedScope.current = riskAcct || 'all'
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (generation !== loadGeneration.current || scope !== requestedScope.current) return
      ...
      const accountChanged = loadedAccount.current !== r.account.accountId
      loadedScope.current = scope
      loadedAccount.current = r.account.accountId
      if (scopeChanged) { dirtyRef.current = {}; setDirty({}) }
      else if (accountChanged) { dirtyRef.current = clearDirty(dirtyRef.current, 'account'); setDirty(dirtyRef.current) }
      ...
      if (generation === loadGeneration.current && scope === requestedScope.current) setError(e.message)
    } finally {
      if (generation === loadGeneration.current) setAccountLoading(false)
    }
  }, [riskAcct])
  useEffect(() => { load() }, [load])
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 @sm:grid-cols-2 @xl:grid-cols-3 gap-x-5 gap-y-1">
                  ...
                  <Field label={`Max accounts per symbol${mark('maxAccountsPerSymbol')}`} anchor="maxAccountsPerSymbol" applied={appliedKeys.has('maxAccountsPerSymbol')} unit="acct" value={risk.maxAccountsPerSymbol} onChange={v => setRisk(r => ({ ...r, maxAccountsPerSymbol: v }))}
                    hint="BOOK-WIDE ceiling: how many ACCOUNTS may hold one symbol in the SAME DIRECTION at once. The per-symbol cap above is per account and cannot see this. Opposite directions are not blocked — they are net-flat across the book. 0 = off."
                    recommend="2. Measured 18-09-2026: NATGAS was 23 of 60 closed trades and −980.84 of the −824.33 total, while ex-NATGAS the book made money at a 1.45 profit factor. Four accounts opened it long in the same minute, one position each, every account inside its own cap of 3." />
