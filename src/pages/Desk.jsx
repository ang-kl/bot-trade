  ...
  return (
    <li className="border-b border-[var(--color-border)] last:border-0 py-px">
      <button type="button" onClick={() => setOpen(o => !o)} aria-expanded={open}
        ...
        <span aria-hidden="true" className="w-2.5 text-(length:--fs-body) shrink-0 text-[var(--color-text-sub)]">{criteria.length ? (open ? '▾' : '▸') : ''}</span>
        {/* Approval is a STATE, not navigation — state-on blue, not accent. */}
        <span className={`w-9 shrink-0 text-(length:--fs-body) font-bold tracking-wide ${ev.approved ? 'text-[var(--color-state-on-text)]' : 'text-[var(--color-warning-text)]'}`}>
          {ev.approved ? 'OK' : 'VETO'}
        </span>
      ...
      {open && criteria.length > 0 && (
        <div className="ml-[52px] mt-0.5 mb-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-(length:--fs-body)">
          {criteria.map(row => (
          ...
          <span className="col-span-2 mt-0.5 text-(length:--fs-body) text-[var(--color-text-sub)] opacity-70">
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <div className="flex items-center gap-1 mb-1.5 flex-wrap">
          ...
          {gridN === 1 && chartSymbols.length > 0 && (
            <select
              ...
              className="glass-inset rounded-[var(--radius-control)] px-2 min-h-[28px] text-(length:--fs-body) font-semibold bg-transparent cursor-pointer max-w-[140px]"
            >
            ...
            {chartSymbols.slice(0, gridN).map(sym => {
              ...
              return (
                <div key={sym} className="min-w-0">
                  <button type="button" className="text-(length:--fs-body) font-bold cursor-pointer hover:underline" onClick={() => { pickSymbol(sym); pickGrid(1) }}>
        ...
        <div className="mt-2 border-t border-[var(--color-border)] pt-1.5 grid gap-x-6 sm:grid-cols-2 lg:grid-cols-3 text-(length:--fs-body)">
          {scans.map(sc => (
            <button
      ...
      >
        ...
        {pulse?.builtAt && (
          <div className="space-y-1.5 text-(length:--fs-body)">
        ...
        {correlation && (
          <div className="space-y-1.5">
            ...
            {(correlation.clusters || []).map(c => (
              <div key={c.key} className="glass-inset rounded-lg p-2 text-(length:--fs-body)">
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{c.label}</span>
                  <span className={`font-bold ${Math.abs(c.net) >= correlation.maxClusterExposure ? 'text-[var(--color-down)]' : ''}`}>
                    net {c.net > 0 ? '+' : ''}{c.net}
                  </span>
                  {Math.abs(c.net) >= correlation.maxClusterExposure && <span className="text-(length:--fs-body) font-semibold text-[var(--color-down)]">AT CAP — new entries in this cluster veto</span>}
                </div>
            ...
            <div className="glass-inset rounded-lg p-2 text-(length:--fs-body) space-y-1">
      ...
      <Section
        ...
        defaultOpen={false}
      >
        {!alphaDecay && <p className="text-(length:--fs-body) text-[var(--color-text-sub)]">No data yet.</p>}
        {alphaDecay && (
          <>
            {/* Band 1 — the auto-bot's LIVE edge */}
            <div className="flex items-center gap-2 mb-1">
              <div className="text-(length:--fs-body) font-semibold">Live edge — auto-bot</div>
              ...
              {labelBackfillNote && <span className="text-(length:--fs-body) text-[var(--color-text-sub)]">{labelBackfillNote}</span>}
            </div>
            {(alphaDecay.strategies?.length ?? 0) === 0 && (
              <p className="text-(length:--fs-body) text-[var(--color-text-sub)]">No closed trades yet — decay is measured from live results; <Link to="/tune" className="text-[var(--color-accent)] underline">arm burn-in in Tune</Link> to build the sample fastest.</p>
            )}
            {(alphaDecay.strategies?.length ?? 0) > 0 && (
              <div className="overflow-x-auto">
            ...
            {/* Band 2 — signal decay */}
            <div className="text-(length:--fs-body) font-semibold mt-3 mb-1">Signal decay — fills vs their signals</div>
            {(alphaDecay.lag_sampled ?? 0) > 0
              ? (
                <p className="text-(length:--fs-body) tabular-nums">
              ...
              : <p className="text-(length:--fs-body) text-[var(--color-text-sub)]">Needs trades that carry their signal timestamp — fills from scanned signals populate this automatically.</p>}

            {/* Band 3 — the OWNER's edge as backtested (per strategy) */}
            <div className="text-(length:--fs-body) font-semibold mt-3 mb-1">Your edge — backtest baseline</div>
            {curBaseline
              ? (
                <>
              ...
              : <p className="text-(length:--fs-body) text-[var(--color-text-sub)]">No baseline stored yet — <Link to="/tune" className="text-[var(--color-accent)] underline">run a backtest in Tune</Link> and your tested edge will appear here for live-vs-tested comparison.</p>}
            ...
            <div className="text-(length:--fs-body) font-semibold mt-3 mb-1">
            ...
            {(alphaDecay.advisories?.length ?? 0) === 0 && <p className="text-(length:--fs-body) text-[var(--color-text-sub)]">Nothing needs attention — edges holding, automation armed.</p>}
            <ul className="text-(length:--fs-body) space-y-1">
      ...
      {health && brokerFlat && positions.length === 0 && (
        <Section id="whynotrades" title="Why No Trades Right Now? card">
          <ul className="text-(length:--fs-body) space-y-1 list-disc pl-5">
