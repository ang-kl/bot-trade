...
- 19-09-2026 08:2x SGT: the re-arm merged as #963 (81c391e); the boot read
  back four `boot_seed` rows, one per account, under the `owner-19-09`
  token.
- 19-09-2026 08:2x SGT: **Wave 5b built and merged** as #964 (9f02eed,
  08:48 SGT) — §K item 17. One `cpp-exec/src/log.hpp` (`sidecar_log::
  logInfo / logWarn / logError`) shared byte-identically with cpp-verify
  and pinned by `agent/services/sidecar-pins.test.js`; every call site in
  both sidecars classified, so boot, feed, session and recorder lines go to
  stdout at severity info and only errors reach stderr (Railway no longer
  paints the whole sidecar log red). Test binary `test_log_streams`. 34 C++
  binaries, ThreadSanitizer, Node gate and CI green before the merge.
  Read-back owed after deploy: both sidecars' boot lines at severity info.
- 19-09-2026 08:5x SGT: **Wave 5a built** (this PR) — §K items 15–16.
  (15) `agent/lib/inflight.js` registers every broker call in flight
  (wsRun, the pooled session, the sidecar client); the watchdog line and
  its action_log row name the oldest call instead of "the loop is stuck";
  `/health` gains `inflight` and `fastMonitor`. A queued call on the pooled
  session is raced against its timeout plus a 30 s queue budget
  (`CTRADER_QUEUE_BUDGET_MS`): an UNSENT request is dropped at the deadline
  and never sent late (cancel-before-send); a STARTED request is never
  rejected by the outer deadline, so a sent order that times out still
  classifies ambiguous and is not resubmitted. The checker's blocker —
  under the pooled session a queued timeout retried into three orders —
  is pinned by a regression test: one placeOrder, one queued_timeout,
  exactly one NEW_ORDER_REQ. The fast monitor writes its skip and busy
  shares over 10 minutes (`fast_monitor_pass_json`) and the goal table
  gains `monitor_cadence` (skip share ≤ 10 %; 19 → 20 rows). (16) An owed
  or `exit_pending` book close is deferred ONLY when the broker's own
  schedule says the market is closed (`hours.source === 'broker'`); the
  heuristic, unknown and error paths attempt the close (the checker's
  second finding: the heuristic deferred open markets). The three
  `[protection]` lines print on change with a 30-minute repeat;
  `pending_fib_orders` is retired in the producer inventory (one boot
  line, the phase not scheduled, heartbeat `retired` flag). And A·8: the
  daily report is generated by the agent (`services/daily-report.js`,
  `GET /state/daily-report`) and posted on the equity-snapshot cadence
  (stamp before work; a failure keeps the last successful `at`). Tests:
  inflight registry, the session's deadline paths, the deferral source
  rule, the print-on-change rule, the retired phase, the report builder
  and poster, wiring pins. Mutations red-then-restored by the maker and
