...
<!-- generated:start -->
Derived by `node scripts/ui-control-inventory.mjs` at HEAD `80945cb` · 119 action call sites · 97 state routes read.
...
| 12 | `src/components/EngineStatusPanel.jsx:206` | setMode | `/actions/entry-mode` | via requestAdmittedBases(db, …), markTickRepush(db, …), requestEntryMode(db, …) | the reply — route returns the effective value (heuristic) | WIRED |
