...
<!-- generated:start -->
Derived by `node scripts/ui-control-inventory.mjs` at HEAD `29a1f6e` · 120 action call sites · 99 state routes read.
...
| 45 | `src/pages/Desk.jsx:849` | (inline) | `/actions/postmortem-sweep` | via runLossPostmortems(db, …), refreshLessonTuning(db, …), executes | the reply — route returns the effective value (heuristic) | WIRED |
| 46 | `src/pages/Desk.jsx:1240` | (inline) | `/actions/llm-budget` | `llm_daily_cost_alert_usd` ✓, `llm_spend_alerted_day` ✓, via setState(db, …) | the reply — route returns the effective value (heuristic) | WIRED |
| 47 | `src/pages/Desk.jsx:1286` | (inline) | `/actions/backfill-label-strategy` | via backfillLabelStrategy(db, …) | the reply — route returns the effective value (heuristic) | WIRED |
