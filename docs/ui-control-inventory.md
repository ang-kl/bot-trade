...
<!-- generated:start -->
Derived by `node scripts/ui-control-inventory.mjs` at HEAD `fbdabbc` · 120 action call sites · 97 state routes read.
...
| 44 | `src/pages/Desk.jsx:305` | (inline) | `/actions/broker-history` | dynamic key(s) — per-account/overlay write, via applyBrokerHistoryMoney(db, …), setState(db, …), executes | the reply — route returns the effective value (heuristic) | WIRED |
| 45 | `src/pages/Desk.jsx:826` | (inline) | `/actions/postmortem-sweep` | via runLossPostmortems(db, …), refreshLessonTuning(db, …), executes | the reply — route returns the effective value (heuristic) | WIRED |
| 46 | `src/pages/Desk.jsx:1217` | (inline) | `/actions/llm-budget` | `llm_daily_cost_alert_usd` ✓, `llm_spend_alerted_day` ✓, via setState(db, …) | the reply — route returns the effective value (heuristic) | WIRED |
| 47 | `src/pages/Desk.jsx:1263` | (inline) | `/actions/backfill-label-strategy` | via backfillLabelStrategy(db, …) | the reply — route returns the effective value (heuristic) | WIRED |
