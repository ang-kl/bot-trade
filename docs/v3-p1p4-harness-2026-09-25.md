...
- **Platform errors** (Railway's JSON 502 "Application failed to respond")
  ...
  listed apart.
- **No window so far is representative.** The only visible tabs were
  `/risk`, `/trade` and `/reasons`; the Desk and the Performance page were
  never visible, and several samples saw no visible tab. Passed readings
  therefore grade Not Verifiable.
