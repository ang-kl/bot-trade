...
- Each account with a plan in ARMED, SENDING, AMBIGUOUS or RECEIVED runs
  ...
  still run.
- An ARMED plan is pre-filtered on the book's marks `{ c, at }`, a price
  with a recorded age: only a mark at most 15 minutes old and more than
  0.25 R short of the trigger skips the broker. A stale, missing or near
  mark, or a ledger row that is no longer open, falls through to the
  authoritative read. `lastScanPrice` is not used: it has no timestamp.
...
- `passHeartbeatAt` and `pass` (fresh within three loop intervals, else
  "unavailable" with the reason), per account when scoped.
...
pass is stale, both show the trigger labelled unavailable, never armed.
