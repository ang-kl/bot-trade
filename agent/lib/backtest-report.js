...
<p class="meta">Strategy: ${esc(payload.strategy === 'cup_handle' ? 'Cup & Handle (breakout, long-only)' : 'Fib 61.8% fade')} · entries: ${esc(payload.entryMode === 'touch' ? 'TOUCH-FILL (resting limit at the level)' : 'close-confirmed (market at next open)')} · run at ${esc(payload.ranAt || '?')} · ${num(payload.bars)} bars requested · filters: ${filters.length ? esc(filters.join(', ')) : 'none'}</p>
<p class="meta">Verdict marks: ✓ GO (blue) · △ GO thin evidence (orange) · ✗ NO-GO (orange, dotted underline)</p>
${sections}
<footer>fib_618_fade walk-forward backtest on real broker bars — generated by the bot-trade agent.</footer>
</body></html>`
}
