...
<p class="meta">Strategy: ${esc(payload.strategy === 'cup_handle' ? 'Cup & Handle (breakout, long-only)' : 'Fib 61.8% fade')} · run at ${esc(payload.ranAt || '?')} · ${num(payload.bars)} bars requested · filters: ${filters.length ? esc(filters.join(', ')) : 'none'}</p>
