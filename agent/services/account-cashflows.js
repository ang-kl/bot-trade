...
export function recordCashflowWindow(db, { accountId, host, currency, from, to, response, receivedAt = Date.now() }) {
  ...
  const rows = (response.depositWithdraw || []).map(r => {
    ...
    // V3 WEB-8: the balance after the operation, kept beside the delta. It is
    // optional evidence: absent or unusable → NULL with balance_source
    // 'broker_api' (read, none given), never a reason to refuse the window.
    const balance = brokerAmount(r.balance, r.moneyDigits)
  ...
  db.transaction(() => {
    ...
    for (const r of rows) {
      ...
      // Two reads of one event disagreeing on its balance: the first stays,
      // and the disagreement is counted rather than refusing the window.
      if (old?.balance != null && r.balance != null && Math.abs(old.balance - r.balance) > 1e-9) balanceConflicts++
      insert.run(String(accountId),host,r.id,r.at,currency,r.delta,r.type,r.kind,receivedAt,r.balance,r.balanceVersion)
    }
