...
test('V3 M3: a P1/P4 row judged against unconfirmed limits is counted as proposed, and one that would be off track is still named', async () => {
  ...
  const would = goals.filter(l => /^ {2}would be off track \(limits proposed, not confirmed; readings on \/state\/goal-table\): /.test(l))
  assert.equal(would.length, 1, `one line of ids, not one line per row: ${goals.join(' | ')}`)
  assert.match(would[0], /: startup_window$/, 'the row is named by its id; its reading stays on the table')
  assert.ok(would[0].length < 120, `a short line (${would[0].length} chars)`)
