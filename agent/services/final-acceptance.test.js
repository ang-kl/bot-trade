...
import { reconcileSegmentManifest, segmentManifestView, VERDICT as R1_VERDICT } from './tick-segment-manifest.js'
import {
...
// The R1 manifest as main's segmentManifestView shapes it (#1130): the policy
// in force with its `since`, the last listing (boot and time), the newest gone
// rows with goneTotal, and restarts[] — one row per restart, with the old
// boot's last listing (prevListingMs) and the open segments torn at the start.
const POLICY_SINCE = iso(T - 30 * H)
const manifestBefore = (policy) => ({ persistence: { policy, since: policy ? POLICY_SINCE : undefined }, lastListing: { atMs: T - 10 * 60_000, bootId: 'boot-a' }, gone: [], goneTotal: 0, restarts: [] })
const manifestAfter = (policy, gone, tornAtStart) => ({
  persistence: { policy, since: policy ? POLICY_SINCE : undefined }, lastListing: { atMs: T + 4 * 60_000, bootId: 'boot-b' }, gone, goneTotal: gone.length,
  restarts: [{ atMs: T + 60_000, prevBootId: 'boot-a', bootId: 'boot-b', prevListingMs: T - 60_000, listedBefore: SEGS.length, lost: gone.filter(g => g.reason === 'lost_restart').length, retired: gone.filter(g => g.reason === 'retired').length, tornAtStart }],
})
...
function drillPair({ policy = 'DURABLE', afterList = SEGS, gone = [], tornAtStart = 0, gaps = 1 } = {}) {
  ...
  const restartAt = T
  const before = {
    '/state/tick-segments': segBody(T - 10 * 60_000, side, SEGS, manifestBefore(policy)),
  ...
  const after = {
    '/state/tick-segments': segBody(T + 4 * 60_000, side, afterList, manifestAfter(policy, gone, tornAtStart)),
...
test("T1 over main's REAL R1 manifest (#1130): a restart is judged by the policy of the boot BEFORE it, not graded until the new boot is listed; a capped gone page is no unrecorded loss; the torn open segment is named", async () => {
  // The manifest bodies are built by R1 itself: the heartbeat's reconcile over
  // two listings (boot-a, then boot-b after the restart) and the view GET
  // /state/tick-segments serves — so every field the drill reads (since,
  // lastListing, restarts[].prevListingMs / tornAtStart, gone, goneTotal) is
  // main's, not a fixture's idea of it.
  const side = 'cpp_exec_demo'
  const SIDE = { name: side, base: 'http://demo.invalid' }
  const SEG = 67_108_872
  const nm = (i) => `seg-${String(1_790_000_000_000 + i * 3_600_000)}-${String(i).padStart(6, '0')}.tks`
  const lister = (ids) => async () => ({ ok: true, enabled: true, spool: '/data/tick', segments: ids.map(i => ({ name: nm(i), bytes: SEG, sealedAtMs: 1_790_000_000_000 + i * 3_600_000 + 3_000_000 })), openBytes: 0, truncated: false })
  const st = (tornAtStart) => ({ enabled: true, recording: true, state: 'RECORDING', segments: { retired: 0, sealed: 0, spoolCapBytes: 2 * 1024 ** 3, segmentBytes: 64 * 1024 * 1024, tornAtStart } })
  ...
  const tA = T - 10 * 60_000   // Node's last listing of boot-a
  const tB = T + 2 * 60_000    // its first listing of boot-b
  const EARLY = '2026-09-25T00:00:00Z'
  async function drill({ afterIds, policy = 'DURABLE', from = EARLY, goneLimit = 100, listNew = true }) {
    const db = initDB(':memory:')
    const P = { sides: { [side]: [{ policy, from, fromMs: Date.parse(from), basis: 'test' }] }, errors: [] }
    await quiet(() => reconcileSegmentManifest(db, SIDE, { status: st(0), bootId: 'boot-a', nowMs: tA, list: lister([1, 2, 3]) }))
    const bm = segmentManifestView(db, { sides: [side], nowMs: tA, policy: P }).sides[side]
    if (listNew) await quiet(() => reconcileSegmentManifest(db, SIDE, { status: st(1), bootId: 'boot-b', nowMs: tB, list: lister(afterIds) }))
    const am = segmentManifestView(db, { sides: [side], nowMs: T + 4 * 60_000, policy: P, goneLimit }).sides[side]
    const { before, after, opts } = drillPair()
    before['/state/tick-segments'] = segBody(tA, side, [1, 2, 3].map(i => ({ name: nm(i), bytes: SEG })), bm)
    after['/state/tick-segments'] = segBody(T + 4 * 60_000, side, afterIds.map(i => ({ name: nm(i), bytes: SEG })), am)
    const r = recorderDrill(before, after, opts)
    return { r, seg: r.checks.find(c => c.name === `${side}.segments`), am }
  }

  // Kept across the restart, DURABLE since before boot-a's last listing: R1 VERIFIED, the drill PASS —
  // and the open segment the new boot tore (restart row tornAtStart 1) is named, never "nothing lost".
  const kept = await drill({ afterIds: [1, 2, 3] })
  assert.equal(kept.am.persistence.verdict, R1_VERDICT.VERIFIED)
  assert.equal(kept.am.restarts[0].tornAtStart, 1)
  assert.equal(kept.seg.verdict, PASS, kept.seg.reason)
  assert.equal(kept.seg.evidence.policySource, 'the restart row'); assert.equal(kept.seg.evidence.prevListingMs, tA); assert.equal(kept.seg.evidence.tornAtStart, 1)
  assert.match(kept.seg.reason, /not covered: 1 open segment\(s\) torn at the restart/)

  // Two sealed segments lost, DURABLE declared before boot-a's last listing: R1 FAILED, the drill FAIL.
  const lost = await drill({ afterIds: [3] })
  assert.equal(lost.am.persistence.verdict, R1_VERDICT.FAILED)
  assert.equal(lost.seg.verdict, FAIL); assert.match(lost.seg.reason, /DURABLE spool: 2 sealed segment\(s\) from before are gone/)

  // The same loss, DURABLE declared AFTER boot-a was last listed (the X1 shape: a volume attached
  // between the old spool's last listing and the restart): R1 does not judge that restart under
  // DURABLE (NOT_VERIFIABLE), and neither does the drill — it names the rule, it does not FAIL.
  const x1 = await drill({ afterIds: [3], from: iso(T) })
  assert.equal(x1.am.persistence.policy, 'DURABLE'); assert.equal(x1.am.persistence.verdict, R1_VERDICT.NOT_VERIFIABLE)
  assert.equal(x1.seg.verdict, NV, 'RED if the drill judges the old boot\'s spool by a policy declared after it was last listed')
  assert.match(x1.seg.reason, /in force since .*after boot boot-a was last listed .*judged by the policy of the boot before it/)
  // An operator-named policy is still taken as given (the caller states the old boot's policy).
  const named = drillPair({ afterList: SEGS.slice(1) })
  named.after['/state/tick-segments'].sides[0].manifest.persistence.since = iso(T)
  assert.equal(recorderDrill(named.before, named.after, named.opts).checks.find(c => c.name === `${side}.segments`).verdict, NV)
  const namedSeg = recorderDrill(named.before, named.after, { ...named.opts, policy: { [side]: 'DURABLE' } }).checks.find(c => c.name === `${side}.segments`)
  assert.equal(namedSeg.verdict, FAIL); assert.equal(namedSeg.evidence.policySource, 'named by the caller')

  // The after body saved before the heartbeat listed boot-b: nothing is classed yet, so a
  // segment missing from the live listing is not yet a loss.
  const early = await drill({ afterIds: [3], listNew: false })
  assert.equal(early.am.lastListing.bootId, 'boot-a')
  assert.equal(early.seg.verdict, NV, 'RED if a segment the manifest has not yet classed is failed as an unrecorded loss')
  assert.match(early.seg.reason, /has not listed boot boot-b yet/)

  // EPHEMERAL, both losses recorded by name: PASS. With the view's gone page cut to 1 row (of
  // goneTotal 2), the name beyond the page may be recorded: NOT_VERIFIABLE, not an unrecorded loss.
  const eph = await drill({ afterIds: [3], policy: 'EPHEMERAL_LOSS_RECORDED' })
  assert.equal(eph.seg.verdict, PASS, eph.seg.reason); assert.match(eph.seg.reason, /2 recorded lost_restart by name/)
  const paged = await drill({ afterIds: [3], policy: 'EPHEMERAL_LOSS_RECORDED', goneLimit: 1 })
  assert.equal(paged.am.goneTotal, 2); assert.equal(paged.am.gone.length, 1)
  assert.equal(paged.seg.verdict, NV); assert.match(paged.seg.reason, /not among the 1 newest gone rows the manifest shows \(of 2\)/)

  // The torn count from the restart row when the recorder sample does not carry it.
  const torn = drillPair({ tornAtStart: 1 })
  for (const b of torn.after['/state/tick-recorder']) delete b.sides[0].status.segments.tornAtStart
