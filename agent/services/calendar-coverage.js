...
// list, whether it is due and why, whether it must wait (a disabled account, a
...
function refreshOf(r) {
  if (!r) return null
  return { ownList: r.map?.ownList === true, enabled: r.enabled === true, due: r.due, dueReason: r.dueReason, blocked: r.blocked, notBefore: r.notBefore,
...
export function buildCalendarCoverage(db, { now = Date.now() } = {}) {
  ...
  return {
    ...
    expiredHolidays: { ...expiredTotal, afterUtcDays: HOLIDAY_EXPIRY_UTC_DAYS,
      basis: 'demanded identities only: non-recurring holiday rows with unreadable bounds, dated at least afterUtcDays UTC days before their own observation; kept in the stored payload, never evaluated, no longer UNKNOWN' },
    export: {
