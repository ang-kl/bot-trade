...
test('counts only NULL-pnl closes older than the grace window', () => {
  ...
  const r = unresolvedPnlSince(db, fxDayStartSql(), { graceMin: 15 })
...
test('account scoping matches the cap it protects', () => {
  ...
  assert.equal(unresolvedPnlSince(db, fxDayStartSql(), { accountId: '111' }).count, 0, 'another account must not block this one')
  assert.equal(unresolvedPnlSince(db, fxDayStartSql(), { accountId: '222' }).count, 1)
  assert.equal(unresolvedPnlSince(db, fxDayStartSql(), { accountId: null }).count, 1, 'portfolio scope sees every account')
})
...
test('a legacy NULL-account row still blocks the account being evaluated', () => {
  ...
  assert.equal(unresolvedPnlSince(db, fxDayStartSql(), { accountId: '111' }).count, 1)
})
...
test('a resolved day still passes, and a real loss still trips the cap', () => {
  const db = mkDb()
  setState(db, 'global_guards_json', JSON.stringify({ portfolioDailyLossUsd: 100 }))
  closeTrade(db, { pnl: -20, minutesAgo: 60 })
  assert.equal(evaluateGlobalGuards(db).ok, true)

  closeTrade(db, { pnl: -95, minutesAgo: 30 })
  const r = evaluateGlobalGuards(db)
