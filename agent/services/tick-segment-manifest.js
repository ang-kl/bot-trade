...
export async function reconcileSegmentManifest(db, side, { status = null, bootId = null, nowMs = Date.now(), list } = {}) {
  ...
  const cur = {
    ...
    // The OPEN segments this boot quarantined as .torn at its start (the
    // recorder's start(): a .tks.open left by the process before it). Not a
    // sealed segment and never listed, so neither policy covers it; recorded
    // on the restart row so a "kept" restart is not read as "nothing lost".
    tornAtStart: num(seg.tornAtStart),
  }
  ...
  db.transaction(() => {
    ...
    if (boot === 'changed') {
      ...
      db.prepare(`INSERT OR IGNORE INTO tick_segment_boots (side, at_ms, prev_boot_id, boot_id, prev_listing_ms, listed_before, bytes_before, survived, retired, lost, lost_bytes, bytes_changed, torn_at_start)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(side.name, nowMs, prev.bootId, cur.bootId, num(prev.atMs), present.length - c.unobserved.length, sum(present.filter(p => !c.unobserved.includes(p.name))),
             c.survivors, retired.length, lost.length, sum(lost), bytesChanged.length, cur.tornAtStart)
    }
    const pending = {}
  ...
  if (boot === 'changed') console.log(`[tick] ${side.name} segment manifest: restart ${prev.bootId} → ${cur.bootId}; ${c.survivors} sealed segment(s) listed again, ${c.gone.filter(g => g.reason === GONE.RETIRED).length} retired, ${c.gone.filter(g => g.reason === GONE.LOST_RESTART).length} lost, ${cur.tornAtStart ?? 'unreported'} open segment(s) torn at start`)
...
export function loadDurabilityPolicy(file = DURABILITY_FILE) {
  ...
  for (const [name, entries] of Object.entries(raw?.sides || {})) {
    ...
    for (const e of Array.isArray(entries) ? entries : []) {
      ...
      sides[name].push({ policy: e.policy, from: e.from ?? null, fromMs, gateway: e.gateway ?? null, basis: e.basis ?? null, pending: e.pending ?? null, whyFrom: e.why_from ?? null })
    }
  }
  return { sides, errors }
}
...
export function persistenceVerdict(db, sideName, { entries = [], state = null, nowMs = Date.now() } = {}) {
  ...
  const base = { policy: inForce.policy, since: inForce.from, basis: inForce.basis, ...(inForce.whyFrom ? { whyFrom: inForce.whyFrom } : {}), declaredNotInForce: declared }
  ...
  let boots = []
  try { boots = db.prepare('SELECT at_ms, prev_boot_id, boot_id, listed_before, survived, retired, lost, lost_bytes, torn_at_start FROM tick_segment_boots WHERE side = ? AND prev_listing_ms >= ? ORDER BY at_ms').all(sideName, inForce.fromMs) } catch { boots = [] }
  ...
  // The open segment is outside both policies (they judge SEALED segments),
  // but a restart that tore one lost those records from every sealed
  // segment: counted beside the verdict and named in its reason, so a
  // VERIFIED is never read as "nothing was lost" (principle 6). Null on a
  // restart row means the new boot's /tick-status did not report it.
  const tornKnown = boots.filter(b => b.torn_at_start != null)
  const tornAtStart = tornKnown.reduce((a, b) => a + (Number(b.torn_at_start) || 0), 0)
  const tornUnreported = boots.length - tornKnown.length
  const facts = { restartsObserved: boots.length, restartsWithSegments: withSegments.length, lostRestart: lost, lostBytes, resized, tornAtStart, tornUnreported }
  const openNote = tornAtStart > 0
    ? `; not covered: ${tornAtStart} open segment(s) torn at ${tornAtStart === 1 ? 'a restart' : 'those restarts'} and quarantined as .torn, their records in no sealed segment (sealing at shutdown is GW-1's clean seal, P8c)`
    : tornUnreported > 0 ? `; not covered: ${tornUnreported} restart(s) did not report whether an open segment was torn` : ''
  if (inForce.policy === DURABILITY.DURABLE) {
    if (lost > 0) return { ...base, ...facts, verdict: VERDICT.FAILED, reason: `${lost} sealed segment(s) (${lostBytes} B) gone at a gateway restart under the DURABLE policy${openNote}` }
    if (resized > 0) return { ...base, ...facts, verdict: VERDICT.FAILED, reason: `${resized} sealed segment(s) listed again with different bytes${openNote}` }
    if (withSegments.length) return { ...base, ...facts, verdict: VERDICT.VERIFIED, reason: `${withSegments.length} gateway restart(s) observed with sealed segments on the spool; every sealed segment was listed again with the same bytes${openNote}` }
    return { ...base, ...facts, verdict: VERDICT.NOT_VERIFIABLE, reason: `no gateway restart with sealed segments on the spool has been observed since ${inForce.from}${openNote}` }
  }
  if (withSegments.length) return { ...base, ...facts, verdict: VERDICT.VERIFIED, reason: `ephemeral by declaration: ${withSegments.length} gateway restart(s) observed, ${lost} sealed segment(s) (${lostBytes} B) lost and each recorded as lost_restart${openNote}` }
  return { ...base, ...facts, verdict: VERDICT.NOT_VERIFIABLE, reason: `no gateway restart with sealed segments on the spool has been observed since ${inForce.from}, so no loss has been recorded yet${openNote}` }
}
