 ...
 * engine. Persistence is judged under the side's declared durability policy,
 * read from the after-body's R1 manifest unless `policy` names it.
 */
export function recorderDrill(before = {}, after = {}, { sides = Object.keys(SIDES), policy = {}, recordingWithinMs = RECORDING_WITHIN_MS } = {}) {
  ...
  for (const side of sides) {
    ...
    const pol = policy[side] ?? aSeg?.manifest?.persistence?.policy ?? null
    if (!Array.isArray(bSeg?.list) || !Array.isArray(aSeg?.list)) {
      ...
      checks.push(check(p('segments'), NV, 'no durability policy is in force for this side (DURABLE or EPHEMERAL_LOSS_RECORDED, agent/config/tick-spool-durability.json)'))
    } else {
      const afterBy = new Map(aSeg.list.map(s => [s.name, s]))
      const goneBy = new Map(arr(aSeg.manifest?.gone).map(g => [g.name, g]))
      ...
      const ev = { policy: pol, before: bSeg.list.length, listedAgain: bSeg.list.length - missing.length - retired.length - lost.length, retired: retired.length, lostRestart: lost.length, missing, resized }
      if (resized.length) checks.push(check(p('segments'), FAIL, `${resized.length} sealed segment(s) listed again with different bytes`, ev))
      else if (pol === 'DURABLE' && (lost.length || missing.length)) checks.push(check(p('segments'), FAIL, `DURABLE spool: ${lost.length + missing.length} sealed segment(s) from before are gone (${[...lost, ...missing.map(m => m.name)].slice(0, 3).join(', ')})`, ev))
      else if (missing.length) checks.push(check(p('segments'), FAIL, `${missing.length} sealed segment(s) from before are gone and the manifest does not record them as lost or retired (${missing.slice(0, 3).map(m => m.name).join(', ')})`, ev))
      else checks.push(check(p('segments'), PASS, pol === 'DURABLE' ? `every sealed segment from before is listed again with the same bytes (${retired.length} retired oldest-first)` : `ephemeral by declaration: ${ev.listedAgain} listed again, ${lost.length} recorded lost_restart by name, ${retired.length} retired`, ev))
    }
    ...
    if (st) {
      ...
      const torn = num(st.segments?.tornAtStart)
