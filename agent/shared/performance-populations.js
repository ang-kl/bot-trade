...
export function reportLedger(report, accountId = 'all') {
  const windows = (report?.windows || []).filter(w => w.ledger).map(w => {
    ...
    return { key: w.key, label: w.label, from: new Date(w.from).toISOString(), to: new Date(w.to).toISOString(),
      ...
      // with its reason. It is never reconstructed from recorded P&L. Its
      // currency groups key on reportCurrency — the one reader the pools
      // above use (V3 WEB-3m) — never on a currency the balance carries.
      ...shape(st), ...ledgerCarry(report.balanceEdges, w.key, accountId, id => reportCurrency(report, id)),
