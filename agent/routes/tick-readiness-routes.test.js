...
test('V3 R1: GET /state/tick-segments carries the segment manifest — names, bytes, what is gone and why — judged under the checked-in durability policy', async () => {
  ...
  try {
    ...
    // Live: DURABLE in force since the volume was attached (21:54 UTC 25-09),
    // and with no restart observed it reads NOT_VERIFIABLE, never VERIFIED.
    assert.equal(asked.manifest.persistence.policy, 'DURABLE')
    assert.equal(asked.manifest.persistence.since, '2026-09-25T21:54:00Z')
    assert.equal(asked.manifest.persistence.verdict, 'NOT_VERIFIABLE')
    assert.deepEqual(asked.manifest.persistence.declaredNotInForce, [])
    // Node's own cache is labelled for what it is: not a second kept copy.
    assert.ok(v.cacheDurability && 'kept' in v.cacheDurability)
  } finally { s.close() }
})
