...
| `EXEC_URL_LIVE` / `EXEC_URL_DEMO` | agent | Optional per-broker-host sidecars. One sidecar process holds ONE broker host for its whole life, so live and demo accounts cannot share one. Unset (the default) = both sides use `EXEC_URL` and behaviour is identical to before. **Not yet safe to split.** The ORDER path is routed by host; the connectivity gate that decides whether an order is ever built is not — `sidecarRoster()` (`loop.js:1082`), the reconcile sweep's gate (`loop.js:2607`), `/state/account-phases` and the heartbeat probe all still read `EXEC_URL`'s roster for every account. Setting `EXEC_URL_DEMO` alone therefore reproduces the 05-08 outage rather than fixing it: the demo accounts stay absent from the roster being consulted and every dispatch is skipped before the routed path is reached. Side-aware probes are the next phase. Two write-side pairings go with it: `probeCppExec`'s two re-push branches (`heartbeat.js:435`, `:451`) resolve their base from `creds.host`, so under a split the probe READS one sidecar and `/connect`s to another — `rosterDrift` then compares one process's roster against the other side's registry, never converges, and re-pushes every probe; and #660's `checkAccountAuthorization` consumes `cpp_exec_health_json.accounts` from that same unrouted probe, so the alarm built for this exact outage would be evaluating cross-side data. `EXEC_URL` must also stay set when these are used — every unrouted caller (`backtestRemote`, `/depth`, the heartbeat probe) resolves to it, and `sidecarRoster` fails OPEN to `null`, so pointing it at nothing stops the gate gating with no signal at all. |
