 ...
 *       lotSize (sent by the keeper from the broker's declaration); without
 *       one the volume is uncompared, never scaled by a guess. Contract 2
...
struct KeeperRecord {
  ...
  std::optional<double> volume;      // LOTS, as the keeper stores them (REAL,
                                     // rounded to 0.01 by its own importer)
  // The symbol's lotSize in the broker's CENTS OF UNITS (ProtoOALightSymbol
  // lotSize, e.g. 10,000,000 for EURUSD), as the keeper's registry holds it
  // from the broker's own declaration. Absent → volume is not compared.
  std::optional<double> lotSize;
...
struct Verdict {
  ...
  std::optional<double> brokerVolume;       // LOTS (needs rec.lotSize)
  std::optional<double> brokerVolumeUnits;  // units, always (centi-units / 100)
...
struct Tolerance {
  ...
  // LOTS. The keeper's importer rounds lots to 0.01 (broker-history-import.js
  // `Math.round((d.volume / meta.lotSize) * 100) / 100`), so half of that
  // step is the keeper's own resolution — the same reasoning as timeMs.
  double volume = 0.005;
 ...
 *  scale — but the keeper stores LOTS, and a lot is a per-symbol quantity
 *  (the broker's lotSize, itself in cents of units). Contract 2 read the four
 *  symbols measured on 18-09-2026 (61200/612, 1500/15, 1800/18, 3610/36.1)
 *  as "divide by 100" — true only because every one of them has lotSize 100
 *  (one unit per lot). The same pass disputed NATGAS (lotSize 1,000,000) and
 *  XPTUSD on exactly that assumption. Units are still reported for the
 *  record; the COMPARISON is in lots, through `KeeperRecord::lotSize`. */
